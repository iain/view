...
You can add a formatter by inheriting from <code>View::Formatter</code>.
The only method you need to implement is the <code>to_s</code> method.

In the class you have access to the following methods:

* <code>value</code>: the object passed in
* <code>options</code>: a filtered hash of options
* <code>all_options</code>: the unfiltered hash
* <code>template</code>: call methods like link_to on this
...
You can use the <code>.as</code> method to specify a different name.
...
To use them, in your code, use the <code>all_options</code> method.
...
See <code>lib/view.rb</code> for information on configuration.
...
Run <code>bundle install</code> and you're ready to go!
...
And run <code>rake gems:install</code> to get it.

Afterwards, include the helper manually, by editing <code>app/helpers/application_helper.rb</code>:

  module ApplicationHelper
    include View::Helper
  end

Caution: some formatters may use new helper methods from Rails 3 and might not work in Rails 2.
